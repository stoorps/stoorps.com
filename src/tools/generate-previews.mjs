...
for (const { catalog } of await readModels()) {
  ...
  if (embedded.id !== catalog.id || embedded.revision !== catalog.revision)
    throw new Error(
      `${catalog.id}: rebuild WASM before generating artwork (catalogue revision mismatch)`,
    );
  ...
  try {
    ...
    console.log(
      `Rendered ${catalog.id} V${catalog.revision} outline (${Math.round(svg.length / 1024)} KB)`,
    );
  } finally {
    model.free();
  }
...
await writeFile(
  "src/generated/previews.ts",
  Object.entries(images)
    .map(([id, file], i) => `import image${i} from './previews/${file}?url';`)
    .join("\n") +
    "\nexport const previewImages: Record<string, string> = {\n" +
    Object.keys(images)
      .map((id, i) => `  ${JSON.stringify(id)}: image${i},`)
      .join("\n") +
    "\n};\n",
);
